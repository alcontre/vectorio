...
const std::uint32_t kDestinationWords =
    kGuardWords + VECTORMMIO_INSTR_SIM_MAX_BURST_WORDS + kMaxWords + kGuardWords;
...
bool run_write_case(std::uint32_t destination_offset, std::uint32_t words)
{
    ...
    volatile std::uint32_t *target = destination + kGuardWords + destination_offset;
    VectorMMIO::write(target, source, words);

    for (std::uint32_t word = 0; word < kDestinationWords; ++word) {
        const bool was_written =
            word >= kGuardWords + destination_offset &&
            word < kGuardWords + destination_offset + words;
        const std::uint32_t expected = was_written
            ? source[word - kGuardWords - destination_offset]
        ...
        if (destination[word] != expected) {
            std::cerr << "max burst " << VECTORMMIO_INSTR_SIM_MAX_BURST_WORDS
                      ...
                      << ", words " << words << ": mismatch at word " << word
                      << std::endl;
            return false;
        }
    }
...
bool run_read_case(std::uint32_t source_offset, std::uint32_t words)
{
    ...
    for (std::uint32_t word = 0; word < kMaxWords; ++word) {
        const std::uint32_t expected = word < words
            ? 0x10000000U + word
            : kDestinationSentinel;
        if (destination[word] != expected) {
            std::cerr << "read max burst "
    ...
    for (std::uint32_t word = 0; word < kDestinationWords; ++word) {
        const bool is_source =
            word >= kGuardWords + source_offset &&
            word < kGuardWords + source_offset + kMaxWords;
        const std::uint32_t expected = is_source
            ? 0x10000000U + word - kGuardWords - source_offset
            : kDestinationSentinel;
        if (source[word] != expected) {
            std::cerr << "read max burst "
                      << VECTORMMIO_INSTR_SIM_MAX_BURST_WORDS
                      << ", source offset " << source_offset
                      << ", words " << words << ": source changed at word "
                      << word << std::endl;
            return false;
        }
    }
